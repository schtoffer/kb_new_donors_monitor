...
az webapp config set --name "$APP_NAME" --resource-group "$RESOURCE_GROUP" --startup-file "gunicorn --bind=0.0.0.0:8000 --timeout 600 wsgi:application" --linux-fx-version "PYTHON|3.13"

if [ $? -ne 0 ]; then
    print_error "Failed to update web app configuration. Please check the error message above."
    exit 1
fi

print_success "Web app configuration updated successfully."
...
print_status "Restarting web app to apply changes..."
az webapp restart --name "$APP_NAME" --resource-group "$RESOURCE_GROUP"

if [ $? -eq 0 ]; then
    print_success "Web app restarted successfully."
    print_status "Please wait a few moments for the app to start up, then visit: https://$APP_URL"
else
    print_error "Failed to restart web app. Please check the error message above."
    exit 1
fi
